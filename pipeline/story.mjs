...
const pick = argSlug
  ? bank.find(s => s.slug === argSlug)
  : nextTypes.map(t => bank.find(s => s.type === t && !used.includes(s.slug))).find(Boolean);
...
const prompt = `You write Instagram story carousels about AI and startups for founders, builders, investors and people who work in tech. This one is an evergreen STORY, not today's news. Tell it like a story: a hook, the setup, the moment everything changed, the number that proves it, the lesson. Use ONLY facts stated in the source text below. Never invent numbers, dates, names or quotes. If a detail in the angle is not in the text, leave it out.
...
Rules:
- LANGUAGE (most important): write like you are telling a smart 14-year-old. Short common words. One idea per sentence. Sentences under 12 words wherever possible. No jargon. Give big numbers a human scale. Indian English. NEVER use em dashes or en dashes. Use periods, commas or colons. Ranges use hyphens.
- headline: max 12 words, the hook. Wrap THE key phrase (a number or the surprise) in ==double equals==. Exactly one highlight.
- 3 or 4 detail slides, each ONE beat of the story in order. Labels: short ("The start", "The problem", "The bet", "The moment", "The number", "What went wrong", "The lesson"). heading: max 14 words, may use one ==highlight==. body: max 2 short sentences.
- category: exactly "${TYPE_LABEL[pick.type]}".
- kicker: a short time anchor from the text, e.g. "1993 · Santa Clara" or "2022 · Bahamas".
- caption, blocks separated by blank lines: 1) HOOK one line max 12 words, not the headline restated; 2) SUBSTANCE 2-3 short paragraphs with extra facts from the text NOT on the slides; 3) one question to the reader ending with 👇; 4) exactly "Follow @wortins.news for AI and startup news that actually sticks."; 5) 7-9 hashtags on one line including #ai #startups #wortins.

Reply with ONLY JSON:
{"type":"news","category":"...","kicker":"...","headline":"...","slides":[{"label":"...","heading":"...","body":"..."},{"label":"...","heading":"...","body":"..."},{"label":"...","heading":"...","body":"..."}],"caption":"..."}`;
